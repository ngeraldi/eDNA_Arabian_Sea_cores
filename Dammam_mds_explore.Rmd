...
## use with log+1 transform - very little difference use above- usually sqrt with wisconsin  
#      mds_dist_fun<- function(df)
#     vegan::metaMDS( vegan::wisconsin(log(  (df[,c(fn:length(colnames(df)))])  +1) ), distance = "bray", trymax=30, autotransform = FALSE)
...
dat2<- dat %>%   # names(dat)   names(dates_pred)
  ...
  mutate(direction.loc4=factor(direction.loc, levels=c("North","Mid-north","Mid-south","South"))) %>% 
  mutate(direction.loc=forcats::fct_recode(direction.loc4, "North"="Mid-north", "South" = "Mid-south")) %>% 
  unite("direction.loc_hab", habitat,direction.loc4 , sep="_", remove = FALSE) %>% 
  mutate(direction.loc_hab=forcats::fct_recode( direction.loc_hab ,"seagrass_Central"="seagrass_Mid-south","seagrass_Central"="seagrass_Mid-north", "mangrove_restored"="mangrove_Mid-north", "mangrove_natural" ="mangrove_Mid-south")) %>% # levels(dat2$direction.loc_hab)
  mutate(direction.loc_hab=factor(direction.loc_hab, levels=c("seagrass_North", "seagrass_Central","seagrass_South", "mangrove_restored", "mangrove_natural") )  ) %>% 
...
## table for NCBI           names(dat2)   unique(dat2$direction.loc)
ncbi<-dat2 %>%      
  filter(!duplicated(sample_num)) %>% 
  select(sample_num, sample_id, type, core, id_dash, ybp,estimated_year, habitat:lon ) %>% 
  mutate(id_dash= sub(".*-", "", id_dash)) %>% 
  rename(core_depth=id_dash, core_id=core)
#  data.table::fwrite(ncbi,paste(dir,export_file,stud_pat,"_ncbi_sample_data.csv",sep=""),row.names=F, sep=",")
...
MDS out-put, 
- not converge if include everything as one


## MDS_plot_all

```{r MDS_plot, eval=FALSE, include=FALSE}
###   all
...
dat3<- dat2 %>%  #  mes<-dat2  mes<-dat3[[1]] names(dat3)  names(mes[,1:50])
  ...
  #
  select(lineage:type,sample_id_u,ybp,type_3cat,ybp_cat_blanks,core,habitat,lat,lon,area,direction.loc,direction.loc_hab, percent_reads, -reads) %>% 
  ### limit data
  filter(type=="sample") %>% 
  filter(primer !="RBCL") %>% 
  filter(primer !="Vert") %>% 
  filter(minboot==70) %>% 
  ## remove bad dated core number 7 when n to s, but 10 original #
  filter( core != 10 ) %>%   # unique(dat2$core)
...
#### 1 -- sum percent reads at selected lineage -- Order now
#### 2 -- sum percent reads over all primer pairs
#### 3 -- then re-calculate percent

## Get euks
dat_euk_base<- dat3 %>%    # names(dat_euk)
  ...
  group_by_at(vars(Superkingdom:direction.loc,direction.loc_hab)) %>% 
  summarize( percent_reads = sum(percent_reads)) %>% 
  ungroup() %>% 
  ### colapse data for primer pairs, get mean per sample  !!!!!!!!!!
  ...
  ## tried summing reads per sample then finding new total percent then updating percent
  #      based on this.  using mean is better
  ## get sum percent per sample    
  #group_by_at(vars(sample_num:direction.loc)) %>% 
  #mutate(sum_percent_per_sample= sum(percent_reads_first))  %>% 
  #ungroup() %>% 
  
  ## calculate new and good percent per sample
  #mutate( percent_reads = (percent_reads/sum_percent_per_sample)*100  ) %>% 
  #ungroup() %>%   # hist(dat_euk$percent_reads)

    # make new lineage
  unite("lineage", Superkingdom:Order, sep = ";", remove = TRUE) 
  
####################################
## added so can use for other catagories
   dat_euk<- dat_euk_base %>% 
    # split by catagory, will run separate mds for each
  split(.$habitat)
...
## get metazoans
dat_metaz_base<- dat3 %>%    # names(dat_metaz)   mes<-dat_metaz[[1]]
  filter(grepl(metaz_pat, Phylum))  %>% 
  filter(percent_reads>0) %>% 
  
  ## recalculate percent per sample for metazoans only
  group_by(primer, sample_num, core, ybp) %>% 
  mutate(sum_metaz_reads = sum(percent_reads)) %>% 
  ungroup() %>% 
  mutate(percent_reads = (percent_reads/sum_metaz_reads)*100   ) %>% 
  
  ### colapse data by specified taxa group ---- now Order --- and sum percent reads   !!!!!!!!!!
  select(-Species, -Genus, -Family, -lineage) %>%   # remove taxa groups and primer related IDs to collapse
  ...
  ## get sum percent per sample   - see explanation above for why not use 
  #group_by_at(vars(sample_num:direction.loc)) %>% 
  #mutate(sum_percent_per_sample= sum(percent_reads_first))  %>% 
  ## calculate new and good percent per sample
  #mutate( percent_reads = (percent_reads/sum_percent_per_sample) *100  ) %>% 
  #ungroup() %>%   # hist(dat_metaz$percent_reads)
  ...
  ####################################
## added so can use for other catagories
  dat_metaz <- dat_metaz_base %>%
  # split by habitat
  split(.$habitat)


## but into one list both lists
...
## by location 
set not to run!!!!!!!!!!! run manually
```{r by_loc, eval=FALSE, include=FALSE}

 ##   if want only eukaryotes  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
dat_both<- dat_euk


## both habitats 
  dat_both<- dat_euk_base %>% 
    # split by catagory, will run separate mds for each   names(dat_euk_base)
    split(list(.$habitat, .$direction.loc) ) %>% 
  set_names("mangrove - north","seagrass - north","mangrove - south","seagrass - south") 
...
##  just seagrass
  dat_both<- dat_euk_base %>% 
    ...
    # split by catagory, will run separate mds for each   names(dat_euk_base)
    split(.$direction.loc ) %>% 
  set_names("seagrass - north","seagrass - south") 
  
## used by habita and location combination - 5 PLOTS  levels(dat_euk_base$direction.loc_hab)
  ##     run lines arond 505 too   !!!!!!!!!!!!!!!!!!!!!!
 dat_both<- dat_euk_base %>% 
    split(.$direction.loc_hab )
...
```{r MDS}
#####       begin getting distance matrix
# set functoin  !!!!!!!!!
   fn<-15 ## first column of taxa - check dat4 lines then related mes

# split or nest by read_transf_type  then spread then dist matrix.
dat4<-dat_both %>%    #  mes<-dat4[[1]]   names(mes[,1:20])
...
MDS_xy2 <- MDS_xy1 %>% 
   ...
   select(group, core, estimated_year,estimated_year_cat,estimated_year_surf_cat,direction.loc_hab,plot_cat, everything())
 
 ## need for hab-location catagories -- messy, not sure what todo
#         MDS_xy2 <- MDS_xy2 %>%  
               mutate(plot_cat=factor(plot_cat, levels=c("seagrass_North", "seagrass_Central","seagrass_South", 
                          "mangrove_restored", "mangrove_natural") )  )  %>% 
               arrange(plot_cat)
 ...
 MDS_xy2_means<- MDS_xy2 %>% 
  ...
  MDS_xy2_arrow<- MDS_xy2_means %>% 
        ...
        select(-habitat)  %>% 
        group_by(plot_cat) %>% 
        slice(2:n()) %>%  # remove first row of each group
        rename(x1=MDS1, y1=MDS2)
    mes<- MDS_xy2_means %>% # calculate 2nd spot used next!!!
        arrange(plot_cat, group) %>% 
        select(-habitat)  %>% 
        group_by(plot_cat) %>% 
...
```{r elipses}
 df_ell <- MDS_xy2 %>%    # names(MDS_xy2)
  ...
  ## remove levels with less than 3 occurances - will end in error
   map(function(x) {
  x<-x %>% 
    group_by(group) %>% 
  filter(n() >= 3)  %>% 
...
# loc_fix_order
!!!!!!!!!!!!!!!   don't run unless!!!!!!!!!!
ONLY for 5 plot hab locations
needed to add to make facet_wrap plot in write direction
```{r crap, eval=FALSE, include=FALSE}
###     only if needed !!!!
vf_spp_all<- vf_spp_all %>%   # levels(MDS_xy2$plot_cat)
     mutate(plot_cat=factor(plot_cat, levels=c("seagrass_North", "seagrass_Central","seagrass_South", "mangrove_restored", "mangrove_natural") )  )  %>% 
  arrange(plot_cat)
stress<- stress %>%   # levels(MDS_xy2$plot_cat)
     mutate(plot_cat=factor(plot_cat, levels=c("seagrass_North", "seagrass_Central","seagrass_South", "mangrove_restored", "mangrove_natural") )  )  %>% 
  arrange(plot_cat)
...
ccol<-rev(c("#D0B990","#B1876B","#92574B","#733136","#541C2D","black","#350D23")[c(1:6)])

legend_loc<-"miss_6th" # "bottomleft"   "topright3rd_plot"  topright1st_plot    miss_6th

#   bottomright1st for 2 euk plots !!
...
# basic plot
 P_mds<- ggplot(MDS_xy2, aes(MDS1, MDS2)) + 
  ...
  facet_wrap(facets=vars(plot_cat), ncol=2, drop=TRUE, scales="free", dir="v", shrink=FALSE) 
 
  # add arrows between mean groups 
    P_mds<- P_mds + geom_segment(MDS_xy2_arrow, mapping = aes(x=x1, xend=x2, y=y1, yend=y2), size=3, alpha=0.5,
                  arrow=arrow(angle = 35, length=unit(0.4,"cm"), type="open", ends = "first"))  +
 # add means
  geom_point(MDS_xy2_means, mapping = aes(MDS1, MDS2, color = group), size=4) 

  # fix theme  need  coord_fixed()+
    # keep names remove/add at end  , strip.text.x = element_blank()
...
## add arrows again  
  ...
  P_mds<- P_mds + labs(color = "Estimated year") +
    theme(legend.position = c(0, 0), 
     ...
     } else if (legend_loc=="miss_6th") {
## fix legend label
  P_mds<- P_mds + labs(color = "Estimated year") +
    theme(legend.position = c(.62, .11), 
      legend.justification = c(0, 0.03), legend.direction="vertical")  
    #P_mds<- P_mds + guides(color=guide_legend(ncol=3))
     }  else if (legend_loc=="bottomright1st") {
## fix legend label
  P_mds<- P_mds + labs(color = "Estimated year") +
    theme(legend.position = c(.05, .55), 
      legend.justification = c(0, 0.03), legend.direction="vertical") 
  }  
...
#    2 plot habitats, euks
#    ggsave(euk1_plot, file=paste(dir,plot_file,"/MDS/mds_habs_euka_plot2.pdf",sep=""), width = 20, height = 35, units = "cm", dpi="print")

#    4 plot habitats, euks, metazoans
#    ggsave(P_mds_species, file=paste(dir,plot_file,"/MDS/mds_habs_time_speciesarrows_4plot2.pdf",sep=""), width = 25, height = 25, units = "cm", dpi="print")

# for location
#    ggsave(P_mds_species, file=paste(dir,plot_file,"/MDS/mds_location_time_5_plot.pdf",sep=""), width = 30, height = 45, units = "cm", dpi="print")


## export env fit data
#    data.table::fwrite(vf_spp_all,paste(dir,export_file,"env_fit_hab_loc_5plot.csv",sep=""),row.names=F, sep=",")

#data.table::fwrite(edna_slice,paste(dir,export_file,export_name,sep=""),row.names=F, sep=",")

```

## tidy habitat with time
```{r tidy_hab_time}


  ## need for hab 2 plots with different location means -- messy, not sure what todo - put if else???
         MDS_xy2 <- MDS_xy2 %>%   # names(MDS_xy2)
                unite("in_plot_cat", direction.loc_hab, estimated_year_surf_cat, sep="_", remove=FALSE)

 MDS_xy2_means<- MDS_xy2 %>% 
    select(group,direction.loc_hab,in_plot_cat,  habitat, MDS1, MDS2)  %>% 
    group_by(group,in_plot_cat, habitat, direction.loc_hab) %>% 
    summarize(MDS1=mean(MDS1),MDS2=mean(MDS2), count=n()) %>% 
     arrange(in_plot_cat, desc(group))
 
 ##   get arrow data   names(MDS_xy2_means)
  MDS_xy2_arrow<- MDS_xy2_means %>% 
        ungroup() %>% 
        arrange(in_plot_cat, habitat ) %>% 
        select(-group, -in_plot_cat) %>% 
        group_by(direction.loc_hab) %>% 
  ...
  mes<- MDS_xy2_means %>% # calculate 2nd spot used next!!!
    ...
    MDS_xy2_arrow$y2<-mes$MDS2
      ...
      MDS_xy2_arrow <- MDS_xy2_arrow %>% 
        arrange(direction.loc_hab, habitat)

      
## redo elipse so different for locations      
 df_ell <- MDS_xy2 %>%    # names(MDS_xy2)
   #mutate(group=estimated_year_cat)  %>%  # name grouping factor
  select(group, everything() ) %>% 
  # split to make original lists
...
## convert o dataframe        unique(df_ell$group)
 df_ell<-bind_rows(df_ell, .id = "in_plot_cat") %>% 
   mutate(group=factor(group, levels=c( "<1500","1500-1799","1800-1959", "1960-1989", "1990-2015","2016" ) ))

```

##hab plot with differnt loc
```{r hab_plot_loc}
ccol<-rev(c("#D0B990","#B1876B","#92574B","#733136","#541C2D","black","#350D23")[c(1:6)])
arr_col<-
...
# mangrove     names(MDS_xy2)
 MDS_xy3 <- MDS_xy2 %>% 
        filter(habitat=="mangrove")
MDS_xy3_means <- MDS_xy2_means %>% 
        filter(habitat=="mangrove")
arr1<-MDS_xy2_arrow %>% #  unique(MDS_xy2_arrow$direction.loc_hab)
    filter(direction.loc_hab=="mangrove_natural")
arr2<-MDS_xy2_arrow %>% #  unique(MDS_xy2_arrow$direction.loc_hab)
    filter(direction.loc_hab=="mangrove_restored")
df_ell2<- df_ell  %>% 
  separate(in_plot_cat,into=c("habitat","cat","year") , sep="_", remove=FALSE) %>%
  filter(habitat=="mangrove" & cat == "restored")
df_ell3<- df_ell  %>% 
  separate(in_plot_cat,into=c("habitat","cat","year") , sep="_", remove=FALSE) %>% 
  filter(habitat=="mangrove" & cat == "natural")
vf_spp2_2<- vf_spp2 %>% 
    filter(plot_cat=="mangrove")
vf_spp2_3<- vf_spp2 %>% 
    filter(plot_cat=="seagrass")

##  start plt
 P_mds  <- ggplot(MDS_xy3, aes(MDS1, MDS2)) + 
  geom_point(aes(color = estimated_year_surf_cat, shape=direction.loc_hab), size=0.9)  +
   scale_color_manual(values=ccol) +
  # add means
  geom_point(MDS_xy3_means, mapping = aes(MDS1, MDS2, color = group, shape=direction.loc_hab), size=5) 
 ...
 # add letter labels and stress
 P_mds<- P_mds + geom_text( data = stress[1,], mapping = aes(x = -Inf, y = Inf, label = "c"),
  hjust   = -0.9, vjust = 1.6) +
          geom_text( data = stress[1,], mapping = aes(x = Inf, y = -Inf, label = sprintf("%0.2f", round(stress_num, digits = 3)) ),
  hjust   = 1.4, vjust = -1.3 , size=3)  
  
## add elipses
#    P_mds <-  P_mds + 
#    geom_polygon(data=df_ell2, aes(x=MDS1, y=MDS2, fill=group), alpha=0.40, size=0, show.legend=FALSE)+scale_fill_manual(values=ccol) + 
#    geom_polygon(data=df_ell3, aes(x=MDS1, y=MDS2, fill=group), alpha=0.40, size=0, show.legend=FALSE) +scale_fill_manual(values=ccol) 
 
  # add arrows between mean groups 
    P_mds<- P_mds + geom_segment(arr2, mapping = aes(x=x1, xend=x2, y=y1, yend=y2), size=3, alpha=0.8, color="grey20",
                  arrow=arrow(angle = 35, length=unit(0.4,"cm"), type="open", ends = "first")) +
          geom_segment(arr1, mapping = aes(x=x1, xend=x2, y=y1, yend=y2), size=3, alpha=0.8, color="grey70",
                  arrow=arrow(angle = 35, length=unit(0.4,"cm"), type="open", ends = "first"))  
            
## fix legend label
  P_mds<- P_mds + labs(color = "Estimated year", shape = " ") +
    theme(legend.position = c(.01, -0.0), legend.margin = margin(0.1,0,0,0, unit="cm"),legend.spacing.y = unit(-0.0, "cm"),
      legend.justification = c(0, 0.00), legend.direction="vertical" ) 
          #

## add arrows with species
P_mds_species    <-  P_mds + geom_segment(data = vf_spp2_2,
               aes(x = 0, xend = NMDS1, y = 0, yend = NMDS2),
               arrow = arrow(length = unit(0.15, "cm")), color = "grey30", lwd=0.4) +
  ggrepel::geom_text_repel(data = vf_spp2_2, aes(x = NMDS1, y = NMDS2, label = sp_label,
                          fontface = font_type), color= vf_spp2_2$word_color,
             cex = 3, direction = "both", segment.size = 0.25, segment.color= "grey60")
  
P_mds_mang  <- P_mds_species
###########################################################################################################################

# seagrass     names(MDS_xy2)
 MDS_xy3 <- MDS_xy2 %>% 
        filter(habitat=="seagrass")
MDS_xy3_means <- MDS_xy2_means %>% 
        filter(habitat=="seagrass")
arr1<-MDS_xy2_arrow %>% #  unique(MDS_xy2_arrow$direction.loc_hab)
    filter(direction.loc_hab=="seagrass_North")
arr2<-MDS_xy2_arrow %>% #  unique(MDS_xy2_arrow$direction.loc_hab)
    filter(direction.loc_hab=="seagrass_Central")
arr3<-MDS_xy2_arrow %>% #  unique(MDS_xy2_arrow$direction.loc_hab)
    filter(direction.loc_hab=="seagrass_South")
df_ell2<- df_ell  %>% 
  separate(in_plot_cat,into=c("habitat","cat","year") , sep="_", remove=FALSE) %>%
  filter(habitat=="seagrass" & cat == "North")
df_ell3<- df_ell  %>% 
  separate(in_plot_cat,into=c("habitat","cat","year") , sep="_", remove=FALSE) %>% 
  filter(habitat=="seagrass" & cat == "Central")
df_ell4<- df_ell  %>% 
  separate(in_plot_cat,into=c("habitat","cat","year") , sep="_", remove=FALSE) %>% 
  filter(habitat=="seagrass" & cat == "South")
vf_spp2_2<- vf_spp2 %>% 
    filter(plot_cat=="seagrass")
...
##  start plot
 ...
 # add letter labels and stress
 P_mds<- P_mds + geom_text( data = stress[2,], mapping = aes(x = -Inf, y = Inf, label = "d"),
  hjust   = -0.9, vjust = 1.6) +
          geom_text( data = stress[2,], mapping = aes(x = Inf, y = -Inf, label = sprintf("%0.2f", round(stress_num, digits = 3)) ),
  hjust   = 1.4, vjust = -1.3 , size=3)  
  
## add elipses
#    P_mds <-  P_mds + 
#    geom_polygon(data=df_ell2, aes(x=MDS1, y=MDS2, fill=group), alpha=0.40, size=0, show.legend=FALSE) +
#    scale_fill_manual(values=ccol) + 
#    geom_polygon(data=df_ell3, aes(x=MDS1, y=MDS2, fill=group), alpha=0.40, size=0, show.legend=FALSE) +
#    scale_fill_manual(values=ccol) + 
#    geom_polygon(data=df_ell4, aes(x=MDS1, y=MDS2, fill=group), alpha=0.40, size=0, show.legend=FALSE) +
#    scale_fill_manual(values=ccol) 
 
  # add arrows between mean groups 
    P_mds<- P_mds + geom_segment(arr3, mapping = aes(x=x1, xend=x2, y=y1, yend=y2), size=3, alpha=0.8, color="grey20",
                  arrow=arrow(angle = 35, length=unit(0.4,"cm"), type="open", ends = "first")) +
           geom_segment(arr2, mapping = aes(x=x1, xend=x2, y=y1, yend=y2), size=3, alpha=0.8, color="grey45",
                  arrow=arrow(angle = 35, length=unit(0.4,"cm"), type="open", ends = "first"))+
             geom_segment(arr1, mapping = aes(x=x1, xend=x2, y=y1, yend=y2), size=3, alpha=0.8, color="grey70",
                  arrow=arrow(angle = 35, length=unit(0.4,"cm"), type="open", ends = "first"))  
...
P_mds_sea<- P_mds_species


## put together

fantastic<- ggpubr::ggarrange(P_mds_mang, P_mds_sea, ncol = 1, nrow = 2, common.legend=FALSE)

fantastic<- gridExtra::arrangeGrob(P_mds_mang, P_mds_sea, ncol = 1)

#  ggsave(fantastic, file=paste(dir,plot_file,"/MDS/mds_2habpplots_location_time.pdf",sep=""), width = 15, height = 25, units = "cm", dpi="print")
...
fantastic<- ggpubr::ggarrange(P_mds_mang + theme(legend.position = c(.05, .05), 
       legend.direction="vertical", plot.margin=unit(c(0.2,0.2,0.3,0.2), "cm")  ) 
        +labs(x=NULL, y=NULL)  + guides(color=FALSE)
                              , P_mds_sea+ theme( plot.margin=unit(c(-0.1,0.2,0.2,0.2), "cm")  ) +labs( y=NULL), ncol = 1, nrow = 2, common.legend=FALSE)
# t = 0, r = 0, b = 0, l = 0


fantastic<- ggpubr::ggarrange(euk1_plot+ theme(legend.position = c(0.0, 0.515) ) , fantastic, ncol = 2, nrow = 1, common.legend=FALSE)

  ggsave(fantastic, file=paste(dir,plot_file,"/MDS/mds_4_hab_euk_location_time.pdf",sep=""), width = 25, height = 25, units = "cm", dpi="print")
...
## OLD demarcation
## cluster all data
used to get cluster with specific number of groups
 TO DO if needed?????  but not yet implemented
